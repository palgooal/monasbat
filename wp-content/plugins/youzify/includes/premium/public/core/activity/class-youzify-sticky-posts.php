...
class Youzify_Activity_Sticky_Posts {
	...
	function is_post_pinned( $activity_id = null ) {
		...
		$component = isset( $_POST['component'] ) ? $_POST['component'] : 'activity';
		...
		$sticky_activities = $this->get_sticky_posts( $component, $item_id );
	...
	function exclude_sticky_posts( $query, $object = null ) {
		...
		$sticky_posts = $this->get_sticky_posts_ids( $component, $item_id );

		if ( empty( $sticky_posts ) ) {
			return $query;
		}

		// Fortce showing activities
		add_filter( 'bp_has_activities', '__return_true' );
	...
	function add_activity_sticksy_posts() {
		...
		$posts_ids = $this->get_sticky_posts_ids( $component, $item_id );
	 ...
	 * Get Sticky Posts.
	 */
	function get_sticky_posts( $component = null, $item_id = null ) {
		...
		// Get Group ID.
		// if ( $component == 'groups' ) {
		// 	$item_id = isset( $_POST['item_id'] ) ? $_POST['item_id'] : bp_get_current_group_id();
		// }

		// Get Sticky Posts ID's
		$posts_ids = apply_filters( 'youzify_get_sticky_posts', youzify_option( 'youzify_' . $component . '_sticky_posts' ), $component, $item_id );

		// Get Group Sticky Posts ID's.
		if ( 'groups' == $component ) {
			$posts_ids = isset( $posts_ids[ $item_id ] ) ? $posts_ids[ $item_id ] : array();
		}
