...
class Youzify_Poll_Activity {
	...
	function activity_result_content() {
		...
		$this->poll_results( $activity_id, $results , $youzify_poll_element );
			        ...
			        <div class="youzify-poll-question youzify-current-bg-color"><i class="fas fa-poll-h"></i><?php echo $youzify_poll_element['question']; ?></div>
				        <div class="youzify-poll-holder"><?php
							...
							if ( ! $get_form && $youzify_show_result == 1 ) {
								// Get Poll Result
								$this->poll_results( $activity->id, $results , $youzify_poll_element );
	...
	function poll_options( $activity_id, $youzify_poll_element, $results, $youzify_user_voted ) {
		...
		// print_r( $youzify_poll_element );
		// print_r( $poll_post_options );
		...
		<div class="youzify-poll-options-holder youzify-poll-type-checkbox">
			<div class="youzify-poll-options youzify-standard-poll-options">
			    ...
			    <?php
					...
					foreach ( $youzify_poll_element[ 'poll_options' ] as $key => $option ) {
						...
						// Show Poll Option.
						echo '<div class="youzify-poll-item-container">' . $poll_img . '<label class="youzify-poll-item-label">' . $input_field  . '<span class="youzify-poll-item-title"> ' . $option['option'] . '</span></label></div>';
	 ...
	 * Get Poll Result.
	 */
	function poll_results( $activity_id, $results, $youzify_poll_element ) {
			    ...
			    <?php
					...
					foreach ( $youzify_poll_element[ 'poll_options' ] as $key => $option ) {
						...
						if ( isset( $option['attachment'] ) ) {
							...
							// Set URL.
							$poll_img = '<a href="' . $img[0] . '" rel="nofollow" class="youzify-poll-image-option" data-youzify-lightbox="youzify-post-' . $activity_id . '"><img loading="lazy" ' . youzify_get_image_attributes( $option['attachment'], 'youzify-medium', 'activity-poll' ) . ' alt=""></a>';;
						...
						echo '<div class="youzify-poll-item">
							' . $poll_img . '
							<div class="youzify-poll-result-content">
		...
		<div class="youzify-poll-actions ">
		    <div class="youzify-see-poll-options youzify-see-standard-poll-options"><i class="fas fa-angle-double-left"></i><?php _e( 'Back to Poll' , 'youzify' ); ?></div>
		</div>
