	...
	function get_suggestions_list( $args ) {
		...
		<div class="youzify-items-list-widget youzify-suggested-groups-widget youzify-list-avatar-circle">
			...
			<div class="youzify-list-item">
				<a href="<?php echo esc_url( $group_url ); ?>" class="youzify-item-avatar"><?php echo bp_core_fetch_avatar( array( 'item_id' => $group_id, 'object' => 'group') ); ?></a>
				<div class="youzify-item-data">
					<a href="<?php echo esc_url( $group_url ); ?>" class="youzify-item-name"><?php echo $group->name; ?></a>
				...
				<div class="youzify-item-action">
					...
					<?php
						...
						if ( 'public' == $group->status ) {
							$add_url = wp_nonce_url( trailingslashit( bp_get_group_url( $group ) . 'join' ), 'groups_join_group' );
						} else if ( 'private' == $group->status ) {
							$add_url = wp_nonce_url( trailingslashit( bp_get_group_url( $group ) . 'request-membership' ), 'groups_request_membership' );
						}

					?>

					<a href="<?php echo esc_url( $add_url ); ?>" class="youzify-list-button youzify-icon-button youzify-add-button">
						<i class="fas fa-sign-in-alt"></i>
					</a>
					<a href="<?php echo esc_url( bp_get_root_url() . "/refuse-group-suggestion/?suggestion_id=" . $group_id . "&_wpnonce=" . wp_create_nonce( 'group-suggestion-refused-' . $group_id ) );?>" class="youzify-list-button youzify-icon-button youzify-close-button">
