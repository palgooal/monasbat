	...
	if ( ! $done ) {
		...
		foreach ( $files as $file ) {
			...
			if ( $file['component'] == 'activity' ) {
				$update['privacy'] = $wpdb->get_var( $wpdb->prepare( "SELECT privacy from {$bp->activity->table_name} WHERE id = %d", $file['item_id'] ) );
			} elseif ( $file['component'] == 'message' ) {
				$update['privacy'] = 'onlyme';
			} elseif( $file['component'] == 'comment' ) {
				$activity_id = $wpdb->get_var( $wpdb->prepare( "SELECT item_id from {$bp->activity->table_name} WHERE id = %d", $file['item_id'] ) );
				$update['privacy'] = $wpdb->get_var( $wpdb->prepare( "SELECT privacy from {$bp->activity->table_name} WHERE id = %d", $activity_id ) );
			}
		...
		foreach ( $posts as $post ) {
			...
			foreach ( $atts as $attachment ) {
				...
				if ( $post['type'] != 'activity_comment' ) {
					$privacy = $wpdb->get_var( $wpdb->prepare( "SELECT privacy from {$bp->activity->table_name} WHERE id = %d", $post['id'] ) );
				} else {
					$ac_id = $wpdb->get_var( $wpdb->prepare( "SELECT item_id from {$bp->activity->table_name} WHERE id = %d", $post['id'] ) );
					$privacy = $wpdb->get_var( $wpdb->prepare( "SELECT privacy from {$bp->activity->table_name} WHERE id = %d", $ac_id ) );
				}
