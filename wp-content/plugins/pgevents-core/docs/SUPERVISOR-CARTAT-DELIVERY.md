...
## 11. Manual Invitation Link — الآن مُفعَّلة فعلياً

> **تحديث:** المرحلة "UI Only" الموصوفة سابقاً في هذا القسم اعتُمِدت، ثم نُفِّذ
> التدفّق الخلفي الفعلي الكامل بالكامل تحت تكليف "Supervisor Manual Invitation
> Link — Secure One-Time Generation". زر "نسخ رابط الدعوة" في
> `templates/event-supervisors.php` يولِّد الآن رابط دعوة حقيقياً ويُنسَخ فعلياً
> إلى الحافظة — لم يعد placeholder بحتاً. راجع التوثيق الكامل والمُفصَّل في
> **`docs/SUPERVISOR-MANUAL-INVITATION-LINK.md`**.
>
> هذه الميزة **منفصلة تماماً** عن تسليم Cartat الموثَّق في §1-§10 أعلاه: لا
> تستدعي `PGE_Cartat_Transport` ولا تعرف بوجوده إطلاقاً، ولا تمسّ أياً من
> ضمانات ترتيب التدوير الآمن الموثَّقة في §4 (تلك الضمانات تبقى سارية بلا أي
> تغيير لمسار تسليم Cartat). سياسة تدوير التوكن هنا مختلفة عمداً (Option A —
> إبطال فوري بعد الالتزام مباشرة، بلا انتظار قبول مزوّد خارجي، لأنه لا يوجد
> مزوّد خارجي في هذا المسار) — التفاصيل الكاملة في المستند الجديد.

## 12. Supervisor Login Architecture — دورة حياة مستقلة، لا علاقة بالقبول

> هذا المستند (§1-§11) يوثِّق **دورة حياة الدعوة** فقط: تفعيل الإسناد لأول
> مرة عبر `invitation_token_hash`، مرة واحدة، للأبد. بعد أن يصبح المشرف
> `active`، لا يزال بحاجة لطريقة تسجيل دخول يومية متكرّرة — رابط الدعوة/
> القبول **لا يُعاد استخدامه لهذا الغرض إطلاقاً**. دورة حياة الدخول الجديدة
> (توكن منفصل تماماً `login_token_hash`، خدمة منفصلة `PGE_Supervisor_Login_
> Service`، مسار مصادقة منفصل `/supervisor/login/{token}/`) موثَّقة بالكامل
> في **`docs/SUPERVISOR-LOGIN-LIFECYCLE.md`**، بما فيها تسليم Cartat لروابط
> الدخول (طبقة منفصلة `PGE_Supervisor_Login_Delivery` فوق نفس
> `PGE_Cartat_Transport` الموثَّق أعلاه، دون أي تعديل عليه).
