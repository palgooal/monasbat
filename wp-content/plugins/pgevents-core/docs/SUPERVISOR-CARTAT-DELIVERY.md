...
## 11. Manual Invitation Link (UI Only) — RC1 Enhancement

الخلفية: يحتاج المضيف بديلاً طارئاً لتسليم دعوة مشرف يدوياً في حال تعذّر
الإرسال عبر واتساب (Cartat). **Manual Invitation Link is intentionally
UI-only in this phase and will be activated in a future enhancement.**

هذه المرحلة أضافت زر "نسخ رابط الدعوة" في `templates/event-supervisors.php`
فقط (نفس شرط الظهور القائم فعلياً لزر "إعادة إرسال" — `invited`/`pending`)،
بلا أي منطق خلفي: لا توليد رابط، لا كشف توكن، لا AJAX، لا نقطة نهاية جديدة.
الضغط عليه يعرض حالياً رسالة معلوماتية فقط عبر مكوّن التنبيه (`showToast`)
القائم أصلاً في نفس القالب. التفعيل الفعلي (توليد رابط حقيقي بلا تدوير توكن
مبكر، نسخ فعلي للحافظة) مؤجَّل بالكامل لمرحلة مستقبلية غير مُحدَّدة بعد،
ولن يمسّ أياً من ضمانات ترتيب التدوير الآمن الموثَّقة في §4 أعلاه.
