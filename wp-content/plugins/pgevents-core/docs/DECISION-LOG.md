...
Decision ID: DEC-009
Date: 2026-08-21
Title: مسؤولية الإرسال المقيَّدة للداعي الإضافي (Scoped Sending Responsibility — Model D2)
Reason: تحليل الجدوى (UX-DECISION-D1) قارن بدائل لمن يملك مسؤولية إرسال دعوات الضيوف الذين
يُنشئهم الداعي الإضافي. القرار المعتمد ("Model D2"): سلطة الإرسال تتبع النطاق (Group/Event)
المخوَّل حالياً للفاعل، لا الجهة التي أنشأت الضيف تاريخياً. سجل تدقيق الإنشاء (المرتبط بـDEC-005)
يبقى إسناداً تاريخياً (Attribution) فقط، ولا يُستخدَم بذاته كمصدر تفويض للإرسال. المالك/الأدمن
يبقى سلطة احتياطية غير مشروطة للإرسال في كل الأحوال (استرداد/إشراف تشغيلي مقصود، لا انتحال هوية
الداعي) — سجل الإرسال يحفظ دوماً الفاعل الفعلي الذي نفَّذ كل عملية إرسال. القناة الخادمية
المُصفَّفة (Queued/Automatic) هي القناة الأولى المعتمدة للتنفيذ؛ قناة روابط واتساب اليدوية مؤجَّلة
لهذه المرحلة. حالة إرسال دائمة لكل ضيف، وحماية من التكرار (Dedup)، مطلوبتان معمارياً. الإلغاء
يُسقِط سلطة الإرسال فوراً؛ الداعي البديل لنفس المجموعة يرث نطاق الإرسال الحالي بغضّ النظر عن مُنشئ
الضيف الأصلي؛ سجلات الإرسال التاريخية لا تتغيّر ولا تُنسَب من جديد.
Approved By: Project Owner
Affected Documents: docs/H1C-EVENT-ACCESS-DESIGN.md §6, §9, §10, §12, §13
Status: Approved — Implementation Pending

== تحديث لاحق (2026-08-22): حسم القرارات التقنية لـModel D2 (D2-TECH-1) — لا تغيير على نص القرار
الأصلي أعلاه ==

تحليل الجدوى التقنية (D2-TECH-1) ترك بعض تفاصيل التنفيذ مفتوحة صراحة. القرارات التالية تحسم تلك
التفاصيل كتوجهات معتمدة، دون تغيير مبدأ Model D2 الأصلي أعلاه:

**(أ) إعادة التحقق من التفويض عند وقت تنفيذ العامل (Worker execution time)، لا عند وقت إدخال
الطلب في الطابور فقط.** إن كان الداعي الإضافي مخوَّلاً وقت إنشاء طلب الإرسال، ثم أُلغيت عضويته، أو
فقد صفة "الداعي الإضافي" المؤهِّلة، أو فقد نطاق المجموعة الحالي المخوَّل — قبل أن ينفِّذ العامل
عملية الإرسال فعلياً — يُمنَع إتمام الإرسال تحت سلطة ذلك الداعي. هذا امتداد مباشر لمبدأ "النطاق
الحالي = السلطة الحالية" الوارد في القرار الأصلي أعلاه، لا مبدأ جديد. **لا يُفهَم هذا كإلغاء
مضمون لطلب نقل (Transport) أُرسل فعلياً للمزوّد بالفعل** — القاعدة تُطبَّق قبل تنفيذ خطوة النقل،
لا بعدها. المالك/الأدمن يبقيان محكومين بسلطتهما التشغيلية الحالية فقط، بنفس المبدأ.

**(ب) الهدف المعماري (Target): أنبوب إرسال دعوات دائم واحد مشترك** بين المالك/الأدمن والداعي
الإضافي — نفس السجل الدائم (Ledger)، نفس قواعد منع التكرار (Dedup)، نفس دلالات الحالة، نفس بنية
الطابور/العامل، ونفس حد النقل (Transport). يختلف التفويض فقط: المالك/الأدمن سلطة تشغيلية على
مستوى المناسبة كاملة؛ الداعي الإضافي مقيَّد بنطاقه الحالي المخوَّل فقط. `actor_user_id`/سجل
الإرسال يحدد الفاعل الفعلي دوماً؛ إرسال المالك/الأدمن سلطة تشغيلية معتمدة، لا انتحال هوية الداعي.
**طابور الإرسال الحالي (Legacy) الخاص بالمالك قد يبقى موجوداً مؤقتاً أثناء التنفيذ التدريجي/الهجرة
— لا يُوثَّق زواله الفوري.** التوجه المعتمد هو أن يتقارب إرسال المالك تدريجياً نحو أنبوب D2 الدائم
بدل بقائه نظاماً موازياً دائم التتبع المفقود.

**(ج) نطاق التنفيذ الأول: Cartat أولاً.** إعادة استخدام حد النقل (Transport) الخادمي القائم لـ
Cartat. لا إعادة تصميم لـUltraMsg ضمن التنفيذ الأولي لـModel D2. UltraMsg يبقى اهتمام
توافق/تكامل مؤجَّلاً لهذه المرحلة فقط — **لا يُوصَف كغير مدعوم في المنتج ككل**، فقط كخارج نطاق
التنفيذ الأول؛ لا يجوز أن تمنع معمارية D2 عمداً إمكانية بناء حد نقل UltraMsg لاحقاً.

**(د) الأداة الأساسية (Primitive) لطبقة الإرسال الخلفية هي إرسال ضيف واحد مُصرَّح به — وليست
"إرسال الكل".** هذه الأداة خادمية السلطة بالكامل: هوية الفاعل من الجلسة الحالية، تفويض
المناسبة/المورد خادمي، الضيف يُحسَم خادمياً، وتخصيص المجموعة الحالي يُعاد التحقق منه خادمياً — لا
وثوق بأي تفويض أو نطاق من العميل. أي عملية جماعية مستقبلية (تحديد يدوي لضيوف، أو كل المؤهلين/غير
المُرسَل لهم) تُبنى فوق نفس أداة التفويض/السجل/الإرسال هذه، بلا دلالات تفويض أو سجل منفصلة. الشكل
الدقيق لواجهة العمليات الجماعية لا يُحسَم بهذا التحديث.

**(هـ) اتجاه السجل الدائم (Ledger) — يبقى التوجه التقني المعتمد في D2-TECH-1 كما هو:** إعادة
استخدام `pge_message_log` (`message_type = invitation`)، بدلالات محاولة/سجل تاريخي غير قابلة
للتعديل (Immutable)، وهوية ضيف دائمة = المناسبة + رقم الهاتف المُطبَّع، مع آليات "تحصين دورة
الحياة" (lifecycle fencing) القائمة أصلاً في نفس الجدول حيثما انطبقت. لا حاجة لجدول رسائل جديد
خاص بـD2 في الوقت الحالي. حالة الرد (RSVP) تبقى منفصلة تماماً عن حالة الإرسال. الإرسال العادي لا
يُعيد الإرسال ضمناً لضيف أُرسل له مسبقاً بنجاح؛ إعادة الإرسال الصريحة فقط تُنشئ محاولة جديدة
مشروعة. المالك والداعي الإضافي يتشاركان السجل نفسه لقرار منع التكرار. لا يُدَّعى "تم التسليم" إلا
إذا أثبت المزوّد فعلياً تسليماً محققاً. **لا تصميم SQL أو Migration ضمن هذا التحديث.**

Status (لهذا التحديث فقط): التوجهات التقنية أعلاه معتمدة (Resolved كقرار)؛ التنفيذ الفعلي لم يبدأ
بعد — ينطبق عليها Status الأصلي لـDEC-009 أعلاه ("Approved — Implementation Pending") دون تغيير.

== تحديث لاحق (2026-08-23): اكتمال التنفيذ التقني (Phase D2، W1-W7) — لا تغيير على نص القرار
الأصلي أعلاه ولا على تحديث D2-TECH-1 ==

التنفيذ الفعلي لكل التوجهات التقنية (أ-هـ) أعلاه اكتمل عبر سبع طبقات متتالية (D2-W1 حتى D2-W7،
راجع docs/MESSAGING-ARCHITECTURE.md §8 للتوثيق المعماري الكامل). لا شيء مما يلي يُغيِّر أو يناقض
القرار الأصلي أو تحديث D2-TECH-1 — إكمال تقني بحت لتوجهات معتمدة سلفاً:

- توجه (أ) (إعادة التحقق وقت تنفيذ العامل): مُنفَّذ حرفياً في `PGE_Invitation_Send_Worker` (D2-W6) —
  إعادة تخويل طازجة تماماً عبر طبقة القرار (Authorization Core) مباشرة قبل أي نقل.
- توجهات (ب)/(د)/(هـ) (أنبوب مشترك، أداة إرسال ضيف واحد، السجل الدائم فوق `pge_message_log`):
  مُنفَّذة عبر `PGE_Invitation_Send_Ledger`/`State`/`Application` (D2-W1 حتى D2-W4) بلا تغيير على
  المبدأ المعتمد.
- توجه (ج) (Cartat أولاً، UltraMsg مؤجَّلة): بلا تغيير — Cartat يبقى ناقل التنفيذ الوحيد.
- **إضافتان تقنيتان لم يفصِّلهما D2-TECH-1 صراحةً، ولا تناقضان أي توجه معتمد أعلاه:**
  1) طبقة طابور تشغيلي مرجعي بحت (`PGE_Invitation_Send_Queue`، D2-W5) ومنسِّق جدولة/استهلاك محدود
     عبر WP-Cron (`PGE_Invitation_Send_Orchestrator`، D2-W7) — تفصيل تنفيذي لعبارة "نفس بنية
     الطابور/العامل" في توجه (ب)، لا مبدأ معمارياً جديداً.
  2) حالة `cancelled` (pre-transport، D2-W6A) لمحاولة أُنهيت قبل أي استدعاء Cartat — إكمال تقني
     صريح لتوجه (أ) (رفض الإتمام عند سقوط التخويل وقت التنفيذ)، وليس توسيعاً لدلالات "تم الإرسال"
     المعتمدة في توجه (هـ).

Status الحالي: **التنفيذ مكتمل** (D2-W1 حتى D2-W7، مع D2-W7 Fix Pass 1 لإيقاظ الجدولة/الاستعادة) —
لا يزال ينطبق Status الأصلي "Approved" (لم يتغيّر مبدأ القرار)؛ إضافة "Implementation Pending" أعلاه
أصبحت تاريخية فقط، ولم تعد تصف الحالة الراهنة. لا DEC-010 مطلوب — لا شيء في التنفيذ يناقض أو يوسِّع
DEC-009/D2-TECH-1 خارج حدود الإكمال التقني الموصوف أعلاه.
```
