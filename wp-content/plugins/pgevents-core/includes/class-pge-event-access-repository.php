...
/**
 * Read-only access to the five event-access tables.
 *
 * The repository is deliberately static: it owns no request or user state,
 * performs no authorization, and executes SQL only when a public read method
 * is called after the schema-readiness and event-scope guards pass.
 */
class PGE_Event_Access_Repository
{
    ...
    {
        return new WP_Error('database_error', 'Unable to read event access storage.');
    }
}
