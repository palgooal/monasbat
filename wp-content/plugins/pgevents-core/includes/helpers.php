...
if (!function_exists('pge_is_host_or_admin')) {
    function pge_is_host_or_admin($event_id)
    {
        ...
        if (!$uid) return false;
        return $uid === (int) get_post_field('post_author', $event_id);
    }
}
...
if (!function_exists('pge_resolve_guest_quota_status')) {
     ...
     *
     * ============================================================================
     * Guest Limit Unification RFC — توضيح حاسم للمعنى (Part B/N)
     * ============================================================================
     * ابتداءً من هذا التوحيد، هذه الدالة هي **المصدر الوحيد** الذي يقرأه
     * PGE_Invitation_Service::create() (نقطة الإنفاذ الموحَّدة الوحيدة لكل
     * مسارات إنشاء الدعوات — يدوي/Bulk Add/Excel، راجع docs/INVITATION-
     * GUEST-LIMIT-ENFORCEMENT.md). معناها هنا محدَّد بدقة: **"سقف عدد سجلات
     * الدعوة" (Invitation Record Limit)** — عدد صفوف `_pge_invited_guests`
     * المسموح بها لمناسبة واحدة. هذا **يختلف تماماً وعمداً** عن الفحص
     * المنفصل الآخر في rsvp-handler.php الذي يستخدم نفس رقم guest_limit
     * المصدري لكن لقياس **سعة الحضور المؤكَّد (Confirmed-Attendance
     * Capacity)** — عدد ردود RSVP "نعم" المسموح بها. لا علاقة تنفيذية بين
     * المفهومين (لا دالة مشتركة، لا قراءة/كتابة مشتركة للبيانات)؛ توحيد
     * هذا التوحيد **لم يمسّ** rsvp-handler.php إطلاقاً ولا يعتزم ذلك — أي
     * قرار لتوحيد المفهومين مستقبلاً قرار معماري/منتجي منفصل تماماً، خارج
     * نطاق هذه المهمة عمداً.
     *
